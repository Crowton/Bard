...
let eval_top texp out =
  ...
  let rec eval (texp: texp) (env: env) (pc: label) (bl: label) : value * label * label = match texp with
    ...
    | RaisedToExp { texp: texp; label: label; _ } ->
        let v, baseLabel, bl' = eval texp env pc bl in
    ...
    | UnOpExp { oper: unOp; texp: texp; canfail: bool; pos: pos } ->
        ...
        in
        let resBl = if canfail then lub bl' expLabel else bl' in
        (resVal, expLabel, resBl)
    
    | IfExp { test: texp; thn: texp; els: texp option; pos: pos } ->
