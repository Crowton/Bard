...
let formatDiff lines =
  ...
  let formatLine line =
    if is_prefix line ~prefix:"+++" then tag "span" ~args:["class","plus3"] line
    else if is_prefix line ~prefix:"---" then tag "span" ~args:["class","minus3"] line
    else if is_prefix line ~prefix:"@@" then tag "span" ~args:["class","at2"] line
    else if is_prefix line ~prefix:"+" then tag "span" ~args:["class","plus"] line
    else if is_prefix line ~prefix:"-" then tag "span" ~args:["class","minus"] line
    else line in
...
let writeResults out_file =
  ...
  Core.Out_channel.write_all out_file ~data:html
